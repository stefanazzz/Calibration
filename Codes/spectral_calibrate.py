...
def crop_and_baseline(
    ...
    t = np.arange(x.size, dtype=np.float64) * dt + shift_sec
...
def explicit_pair_key(path: Path) -> str:
    return re.sub(r"[_-]?ch\d+$", "", path.stem, flags=re.IGNORECASE)


def paths_by_pair_key(paths: list[Path], label: str) -> dict[str, Path]:
    out = {}
    for path in paths:
        key = explicit_pair_key(path)
        if key in out:
            print(
                f"Warning: duplicate {label} pairing key {key!r}; "
                f"keeping {out[key].name}, skipping {path.name}"
            )
            continue
        out[key] = path
    return out
...
def pair_paths_from_globs(globs: list[str]) -> list[tuple[Path, Path]]:
    ...
    file1_by_key = paths_by_pair_key(file1_paths, "file1")
    file2_by_key = paths_by_pair_key(file2_paths, "file2")

    file1_keys = set(file1_by_key)
    file2_keys = set(file2_by_key)
    for key in sorted(file1_keys - file2_keys):
        print(f"Warning: missing file2 partner for {file1_by_key[key].name}")
    for key in sorted(file2_keys - file1_keys):
        print(f"Warning: missing file1 partner for {file2_by_key[key].name}")

    return [(file1_by_key[key], file2_by_key[key]) for key in sorted(file1_keys & file2_keys)]
...
def band_rows(
    ...
    power_ok: np.ndarray,
    file1_power: np.ndarray,
    file2_power: np.ndarray,
    ...
    for f0, f1 in bands:
        ...
        energy_band = (
            band
            & np.isfinite(file1_power)
            & np.isfinite(file2_power)
            & (file1_power >= 0)
            & (file2_power > 0)
        )
        if np.any(energy_band):
            energy_ratio = float(
                math.sqrt(float(np.sum(file1_power[energy_band])) / float(np.sum(file2_power[energy_band])))
            )
        else:
        ...
        rows.append(
            {
                ...
                "energy_ratio_file1_per_file2": energy_ratio,
...
def main() -> None:
    ...
    ap.add_argument(
        ...
        default=0.0,
        help=(
            "Time shift applied to file1 before baseline correction and spectral "
            "window cropping. Positive values move file1 later; negative values "
            "move file1 earlier. Default: 0.0."
        ),
    )
    ap.add_argument(
        "--amp-file1",
    ...
    for row in rows:
        ...
        energy_ratio = row["energy_ratio_file1_per_file2"]
        energy_ratio_s = f"{energy_ratio:.6e}" if np.isfinite(energy_ratio) else "nan"
        print(
            f"  {row['f0_hz']:9.0f}-{row['f1_hz']:<9.0f} Hz  "
            f"{value_s}  energy={energy_ratio_s}  coh={row['coherence_median']:.3f}  "
