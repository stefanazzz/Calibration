...
def main():
    ...
    #
    # global:
    #   Previous behaviour: each trace is normalized by its own maximum absolute
    #   amplitude inside the cropped/plot window. Therefore each trace reaches
    #   +/-1 in its own axis, unless it is flat.
    #
    # polarity:
    #   New behaviour: for each trace, compute the positive-peak amplitude and
    #   negative-trough magnitude. Across both traces, decide whether the
    #   positive or negative polarity is dominant. Then normalize each trace by
    #   its own peak of that chosen polarity. This keeps the C-factor calculation
    #   based on comparable-polarity peaks. The opposite polarity may exceed
    #   +/-1 if it is larger than the selected-polarity peak in a given trace.
    if args.norm_mode == "global":
        norm_polarity = "global abs"
        scale_file1 = float(np.max(np.abs(x_file1_corr)))
        scale_file2 = float(np.max(np.abs(x_file2_corr)))

    elif args.norm_mode == "polarity":
        ...
        raise SystemExit(f"Unknown normalization mode: {args.norm_mode}")
