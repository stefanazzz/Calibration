...
def main():
    ...
    t0_full, x2_corr_full = data_list[0]
    t1_full, x3_corr_full = data_list[1]
    ...
    # Important change: crop first, then normalize using only the plotted samples.
    t0, x2_corr, x2_norm, max_amp1, invf1 = crop_and_normalize(
        t0_full, x2_corr_full, plot_tmin, plot_tmax
    )
    t1, x3_corr, x3_norm, max_amp2, invf2 = crop_and_normalize(
        t1_full, x3_corr_full, plot_tmin, plot_tmax
    )
    ...
    # Ch02 and Ch03 are now inverse normalization factors from the plotted interval only.
    Ch02 = invf1
    Ch03 = invf2
    try:
